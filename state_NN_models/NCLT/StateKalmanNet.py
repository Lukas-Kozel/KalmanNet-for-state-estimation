...
class StateKalmanNetNCLT(nn.Module):
    ...
    def __init__(self, system_model, device, 
        ...
        super(StateKalmanNetNCLT, self).__init__()
    ...
    def step(self, y_t_raw, u_t=None): # Přidal jsem u_t pro podporu řízení
        """
        ...
        # === UPDATE START: Handling Chybějících Dat (NaN) ===
        
        # A. Vytvoření masky (1 = máme data, 0 = chybí/NaN)
        # Stačí se podívat na první sloupec y (pokud chybí jedno, chybí obvykle celé měření)
        mask = ~torch.isnan(y_t_raw[:, 0]).view(-1, 1) # [B, 1]

        # B. Nahrazení NaN bezpečnou hodnotou (např. y_pred nebo 0)
        # Abychom mohli počítat rozdíly bez chyb, nahradíme NaN v y_t_raw
        # Pokud chybí měření, tváříme se, že jsme naměřili přesně to, co jsme čekali (y_pred)
        # Tím pádem bude inovace = 0.
        y_t_safe = torch.where(mask, y_t_raw, y_pred_raw)
        
        # C. Inovace (Residual)
        innovation = y_t_safe - y_pred_raw
        ...
        # === APLIKACE MASKY NA GAIN ===
        # Pokud nemáme měření, K musí být 0.
        # [B, m, n] * [B, 1, 1] (broadcasting)
