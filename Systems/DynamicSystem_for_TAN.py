...
class DynamicSystemTAN:
    ...
    def calculate_terrain_gradient(self, x_in):
        """
        Vypočítá gradient terénu (d(Výška)/dx, d(Výška)/dy) v bodě x_in.
        Robustní verze, která funguje i uvnitř torch.no_grad() bloku.
        """
        # 1. Musíme vynutit sledování gradientů, i když jsme ve validační fázi
        with torch.enable_grad():
            
            # 2. Příprava vstupu (musí vyžadovat gradient)
            # .detach() je důležitý, abychom se odpojili od předchozí historie (GRU)
            x_batch = self._prepare_input(x_in).detach().clone()
            x_batch.requires_grad_(True)
            
            # 3. Zavoláme funkci měření h()
            # Díky enable_grad() se zde vytvoří lokální výpočetní graf
            y_out = self.h(x_batch)
            z_height = y_out[:, 0] # Předpoklad: Index 0 je výška
            
            # 4. Spočítáme gradient
            gradients = torch.autograd.grad(
                outputs=z_height, 
                inputs=x_batch,
                grad_outputs=torch.ones_like(z_height),
                create_graph=False, # Graf pro výsledek nepotřebujeme (nepočítáme 2. derivaci)
                retain_graph=False,
                only_inputs=True
            )[0]
            
            # 5. Extrakce a ořezání
            terrain_grad = gradients[:, :2] # [dx, dy]
            
            # Clamp pro numerickou stabilitu (např. max sklon 5m na metr)
            terrain_grad = torch.clamp(terrain_grad, -5.0, 5.0)
